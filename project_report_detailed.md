...
## Complete Technical & Functional Architecture Report (Detailed Architecture Diagram)
...
### System Topography & Full Connections Diagram

The diagram below shows the complete topology, including routing, service discovery registry, configuration mapping, PostgreSQL database schemas, asynchronous messaging, and monitoring hooks (tracing and metrics collection).

```mermaid
graph TD
    UI[Angular Frontend - Port 4200] -->|HTTP Requests| Gateway[API Gateway - Port 8080]
    
    %% Config Server Connections
    Config[Config Server - Port 8888] -.->|Centralized Configs| Gateway
    Config -.->|Centralized Configs| Auth[Auth Service - Port 8081]
    Config -.->|Centralized Configs| Delivery[Delivery Service - Port 8082]
    Config -.->|Centralized Configs| Tracking[Tracking Service - Port 8083]
    Config -.->|Centralized Configs| Admin[Admin Service - Port 8084]
    
    %% Service Discovery Connections
    Eureka[Eureka Registry - Port 8761] <--->|Discovery| Gateway
    Eureka <--->|Discovery| Auth
    Eureka <--->|Discovery| Delivery
    Eureka <--->|Discovery| Tracking
    Eureka <--->|Discovery| Admin

    %% Routing
    Gateway -->|/auth/**| Auth
    Gateway -->|/deliveries/**| Delivery
    Gateway -->|/tracking/**| Tracking
    Gateway -->|/admin/**| Admin

    %% Database Ingestion
    DB[(PostgreSQL - Port 5432)] <---> Auth
    DB <---> Delivery
    DB <---> Tracking
    DB <---> Admin

    %% Inter-service Feign Communication
    Admin -->|Feign client with JWT propagation| Auth
    Admin -->|Feign client with JWT propagation| Delivery

    %% Asynchronous communication via RabbitMQ
    Delivery -->|Publish: courier-exchange| RMQ{RabbitMQ - Port 5672}
    RMQ -->|Consume: tracking-queue| Tracking

    %% Infrastructure & DevOps
    Zipkin[Zipkin Server - Port 9411] -.->|Trace Collection| Auth & Delivery & Tracking & Admin & Gateway
    Prom[Prometheus Server - Port 9090] -.->|Scrapes Actuator Metrics| Auth & Delivery & Tracking & Admin & Gateway
```
